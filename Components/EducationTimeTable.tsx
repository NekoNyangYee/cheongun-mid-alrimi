  ...
  return (
    <WrapTimeTableContainer>
      ...
      <SelectGradeClassNMContainer>
        ...
        <SelectOption
          value={`${selection.GRADE}-${selection.CLASS_NM}`}
          onChange={(e) => {
            const [grade, classNm] = e.target.value.split('-');
            handleSelection(grade, classNm);
          }}
        >
          {availableClasses.map((cls, index) => (
            <option key={index} value={`${cls.GRADE}-${cls.CLASS_NM}`}>
